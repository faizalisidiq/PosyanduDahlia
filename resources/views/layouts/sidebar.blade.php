<aside
    ...
    <div class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
        ...
        @if (Auth::user()->staff && in_array(Auth::user()->staff->role, ['ketua-kader', 'anggota-kader']))
            ...
            @if (Auth::user()->staff->role === 'ketua-kader')
                <a href="{{ route('archives.index') }}"
                    class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors duration-200 mb-2
                {{ request()->routeIs('archives.*') ? 'bg-teal-50 text-teal-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg class="w-5 h-5 mr-3 {{ request()->routeIs('archives.*') ? 'text-teal-600' : 'text-gray-400' }}"
                        fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="flex-1 h-0 overflow-y-auto mt-2">
            <nav class="px-2 space-y-1">
                ...
                @if (Auth::user()->staff && in_array(Auth::user()->staff->role, ['ketua-kader', 'anggota-kader']))
                    ...
                    @if (Auth::user()->staff->role === 'ketua-kader')
                        <a href="{{ route('archives.index') }}"
                            class="group flex items-center px-4 py-3 text-base font-medium rounded-xl transition-all duration-200 mb-2
                            {{ request()->routeIs('archives.*') ? 'bg-teal-50 text-teal-700 shadow-sm' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                            <svg class="mr-4 h-6 w-6 {{ request()->routeIs('archives.*') ? 'text-teal-600' : 'text-gray-400' }}"
                                fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
                            </svg>
