            ...
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-semibold {{ $elderly->gender === 'male' ? 'bg-blue-50 text-blue-700 border border-blue-100' : 'bg-pink-50 text-pink-700 border border-pink-100' }}">
                            {{ $elderly->gender === 'male' ? 'Laki-laki' : 'Perempuan' }}
                        </span>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-semibold bg-purple-50 text-purple-700 border border-purple-100">
                            {{ \Carbon\Carbon::parse($elderly->birth_date)->age }} Tahun
                        </span>
                    </div>
                    <p class="text-base text-gray-500">NIK: {{ $elderly->identity_number }} | BPJS: {{ $elderly->social_security_number ?? '-' }}</p>
                </div>
            </div>
        ...
        <div class="border-b border-gray-200">
            <nav class="flex space-x-6 -mb-px" aria-label="Tabs">
                ...
                <button @click="activeTab = 'screening'"
                    :class="activeTab === 'screening' ? 'border-teal-500 text-teal-600 font-bold' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                    class="py-4 px-1 border-b-2 font-medium text-base whitespace-nowrap transition-all focus:outline-none">
                    Screening ILP
                </button>
            </nav>
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <p class="text-gray-900 font-medium font-mono tracking-wide">{{ $elderly->identity_number }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Tempat, Tanggal Lahir</label>
                                <p class="text-gray-900 font-medium break-words">{{ $elderly->birth_place ?? '-' }}, {{ $elderly->birth_date->translatedFormat('d F Y') }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Usia</label>
                                <p class="text-gray-900 font-mono">{{ \Carbon\Carbon::parse($elderly->birth_date)->age }} Tahun</p>
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Jenis Kelamin</label>
                                <p class="text-gray-900 font-medium">{{ $elderly->gender === 'male' ? 'Laki-laki' : 'Perempuan' }}</p>
                            </div>
                            <div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div class="p-6 border-b border-gray-100 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-1">Daftar screening kesehatan Integrasi Layanan Primer (ILP) lansia.</p>
                    </div>
                        ...
                        <tbody class="divide-y divide-gray-100 text-base text-gray-700">
                            @forelse($screenings as $record)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-gray-500 max-w-xs truncate" title="{{ $record->results['note'] ?? '-' }}">
                                        {{ $record->results['note'] ?? '-' }}
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-right space-x-2">
                                        <a href="{{ route('ilp-screenings.show', $record) }}" class="text-teal-600 hover:text-teal-700 font-medium">Detail</a>
                                        <a href="{{ route('ilp-screenings.edit', $record) }}" class="text-blue-600 hover:text-blue-700 font-medium">Ubah</a>
                                        <form action="{{ route('ilp-screenings.destroy', $record) }}" method="POST" class="delete-form inline-block">
                                            @csrf
                                            @method('DELETE')
