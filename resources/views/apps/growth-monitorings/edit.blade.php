...
<div class="w-full mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-[0_0_20px_rgba(0,0,0,0.05)] border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100">
            ...
            <p class="text-sm text-gray-500">Perbarui hasil pengukuran berat dan tinggi badan anak.</p>
        </div>
        ...
        <form action="{{ route('growth-monitorings.update', $growthMonitoring) }}" method="POST" class="p-6 space-y-8" autocomplete="off">
            ...
            <div class="space-y-4">
                <h3 class="text-sm font-semibold text-teal-700 uppercase tracking-wider border-b border-gray-100 pb-2">Detail Pengukuran</h3>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Child Field -->
                    <div class="w-full">
                        <label for="child_id" class="block text-sm font-medium text-gray-700 mb-1">Nama Anak <span class="text-red-500">*</span></label>
                        <select name="child_id" id="child_id" required
                            ...
                            @foreach($childrens as $child)
                                <option value="{{ $child->id }}" {{ old('child_id', $growthMonitoring->child_id) == $child->id ? 'selected' : '' }}>{{ $child->name }} - (Ibu: {{ $child->mother->name }})</option>
                            @endforeach
                        </select>
                         @error('child_id')
                    ...
                    <!-- Arm Circumference -->
                    <div class="w-full">
                        <label for="arm_circumference" class="block text-sm font-medium text-gray-700 mb-1">Lingkar Lengan Atas / LILA (cm)</label>
                    ...
                    <!-- Head Circumference -->
                    <div class="w-full">
                        <label for="head_circumference" class="block text-sm font-medium text-gray-700 mb-1">Lingkar Kepala (cm)</label>
            ...
            <div class="flex items-center justify-end space-x-3 pt-4 border-t border-gray-100">
                <a href="{{ route('growth-monitorings.index') }}" class="px-4 py-2 bg-white border border-gray-200 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors">
                    Batal
                </a>
