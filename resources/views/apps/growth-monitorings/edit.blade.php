...
@section('content')
    <div class="w-full mx-auto space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-[0_0_20px_rgba(0,0,0,0.05)] border border-gray-100 overflow-hidden">
            ...
            <form action="{{ route('growth-monitorings.update', $growthMonitoring) }}" method="POST" class="p-6 space-y-8"
                autocomplete="off">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Child Field -->
                        <div class="w-full">
                            <label for="child_id" class="block text-base font-medium text-gray-700 mb-1">Nama Anak <span
                            ...
                            <select id="child_id" disabled
                                ...
                                @foreach ($childrens as $child)
                                    @if ($growthMonitoring->child_id == $child->id)
                                        <option value="{{ $child->id }}" selected>{{ $child->name }} - (Ibu:
                            ...
                            <select name="staff_id" id="staff_id" required
                                ...
                                @foreach ($staffs as $staff)
                                    <option value="{{ $staff->id }}"
                                        {{ old('staff_id', $growthMonitoring->staff_id) == $staff->id ? 'selected' : '' }}>
                                        {{ $staff->user->name }} - {{ $staff->role }}</option>
                                @endforeach
                            </select>
                            @error('staff_id')
                        ...
                        <div class="w-full">
                            ...
                            <input type="date" name="checkup_date" id="checkup_date"
                                value="{{ old('checkup_date', $growthMonitoring->checkup_date->format('Y-m-d')) }}"
                                required
                        ...
                        <!-- Weight -->
                        <div class="w-full">
                            <label for="weight" class="block text-base font-medium text-gray-700 mb-1">Berat Badan (kg)
                                <span class="text-red-500">*</span></label>
                            <input type="number" step="0.1" name="weight" id="weight"
                                value="{{ old('weight', $growthMonitoring->weight) }}" required
                        ...
                        <div class="w-full">
                            ...
                            <input type="number" step="0.1" name="height" id="height"
                                value="{{ old('height', $growthMonitoring->height) }}" required
                        ...
                        <!-- Head Circumference -->
                        <div class="w-full">
                            <label for="head_circumference" class="block text-base font-medium text-gray-700 mb-1">Lingkar
                                Kepala (cm)</label>
                        ...
                        <div class="w-full md:col-span-2">
                            <label for="note" class="block text-base font-medium text-gray-700 mb-1">Catatan
                                Tambahan</label>
