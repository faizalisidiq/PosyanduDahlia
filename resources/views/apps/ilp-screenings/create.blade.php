    ...
    <div class="w-full mx-auto space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-[0_0_20px_rgba(0,0,0,0.05)] border border-gray-100 overflow-hidden">
            <div class="p-6 border-b border-gray-100">
                ...
                <p class="text-base text-gray-500">Catat hasil screening kesehatan Integrasi Layanan Primer.</p>
            </div>
            ...
            <form action="{{ route('ilp-screenings.store') }}" method="POST" class="p-6 space-y-8" autocomplete="off"
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Subject Type -->
                        <div class="w-full">
                            <label for="subjectable_type" class="block text-base font-medium text-gray-700 mb-1">Tipe Pasien
                                <span class="text-red-500">*</span></label>
                            @if(isset($preselectedType) && $preselectedType)
                                <input type="hidden" name="subjectable_type" value="{{ $preselectedType }}">
                                <select id="subjectable_type" disabled
                                    class="block w-full rounded-lg border-gray-200 bg-gray-100 text-gray-500 shadow-sm sm:text-base p-2.5 transition-all">
                                    <option value="App\Models\Mother" {{ $preselectedType == 'App\Models\Mother' ? 'selected' : '' }}>Ibu</option>
                                    <option value="App\Models\Children" {{ $preselectedType == 'App\Models\Children' ? 'selected' : '' }}>Anak</option>
                                    <option value="App\Models\Elderly" {{ $preselectedType == 'App\Models\Elderly' ? 'selected' : '' }}>Lansia</option>
                                </select>
                            @else
                                <select name="subjectable_type" id="subjectable_type" required onchange="toggleSubjectList()"
                                    class="block w-full rounded-lg border-gray-200 bg-gray-50 text-gray-900 focus:bg-white focus:border-teal-500 focus:ring-teal-500 shadow-sm sm:text-base p-2.5 transition-all @error('subjectable_type') border-red-500 bg-red-50 @enderror">
                        ...
                        <!-- Subject ID (Dynamic) -->
                        <div class="w-full">
                            <label for="subjectable_id" class="block text-base font-medium text-gray-700 mb-1">Nama Pasien
                                <span class="text-red-500">*</span></label>

                            @if(isset($preselectedId) && $preselectedId)
                                <input type="hidden" name="subjectable_id" value="{{ $preselectedId }}">
                                <select id="preselected_subject_id" disabled
                                    class="block w-full rounded-lg border-gray-200 bg-gray-100 text-gray-500 shadow-sm sm:text-base p-2.5 transition-all">
                                    @if($preselectedType == 'App\Models\Mother')
                                        @foreach($mothers as $mother)
                                            @if($preselectedId == $mother->id)
                                                <option value="{{ $mother->id }}" selected>{{ $mother->name }} (NIK: {{ $mother->identity_number }})</option>
                                            @endif
                                        @endforeach
                                    @elseif($preselectedType == 'App\Models\Children')
                                        @foreach($childrens as $child)
                                            @if($preselectedId == $child->id)
                                                <option value="{{ $child->id }}" selected>{{ $child->name }} (Ibu: {{ $child->mother->name }})</option>
                                            @endif
                                        @endforeach
                                    @elseif($preselectedType == 'App\Models\Elderly')
                                        @foreach($elderlies as $elderly)
                                            @if($preselectedId == $elderly->id)
                                                <option value="{{ $elderly->id }}" selected>{{ $elderly->name }} (NIK: {{ $elderly->identity_number }})</option>
                                            @endif
                                        @endforeach
                                    @endif
                                </select>
                            @else
                                ...
                                <select name="subjectable_id" id="mother_list"
                                    class="hidden w-full rounded-lg border-gray-200 bg-gray-50 text-gray-900 focus:bg-white focus:border-teal-500 focus:ring-teal-500 shadow-sm sm:text-base p-2.5 transition-all">
                        ...
                        <!-- Staff Field -->
                        <div class="w-full">
                            <label for="staff_id" class="block text-base font-medium text-gray-700 mb-1">Petugas Pemeriksa
                                <span class="text-red-500">*</span></label>
                            <select name="staff_id" id="staff_id" required
                                ...
                                @foreach($staffs as $staff)
                                    <option value="{{ $staff->id }}" {{ old('staff_id', auth()->user()->staff->id ?? '') == $staff->id ? 'selected' : '' }}>
                        ...
                        <!-- Checkup Date -->
                        <div class="w-full">
                            <label for="checkup_date" class="block text-base font-medium text-gray-700 mb-1">Tanggal
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <!-- Weight -->
                        <div class="w-full">
                            <label for="results_weight" class="block text-base font-medium text-gray-700 mb-1">Berat Badan
                                (kg)</label>
                        ...
                        <!-- Height -->
                        <div class="w-full">
                            <label for="results_height" class="block text-base font-medium text-gray-700 mb-1">Tinggi Badan
                                (cm)</label>
                        ...
                        <!-- Waist -->
                        <div class="w-full">
                            <label for="results_waist" class="block text-base font-medium text-gray-700 mb-1">Lingkar Perut
                                (cm)</label>
                        ...
                        <!-- Blood Pressure -->
                        <div class="w-full">
                            <label for="results_blood_pressure" class="block text-base font-medium text-gray-700 mb-1">Tekanan
                                Darah (mmHg)</label>
                        ...
                        <!-- Blood Sugar -->
                        <div class="w-full">
                            <label for="results_blood_sugar" class="block text-base font-medium text-gray-700 mb-1">Gula Darah
                                (mg/dL)</label>
                        ...
                        <!-- Uric Acid -->
                        <div class="w-full">
                            <label for="results_uric_acid" class="block text-base font-medium text-gray-700 mb-1">Asam Urat
                                (mg/dL)</label>
                        ...
                        <!-- Cholesterol -->
                        <div class="w-full">
                            <label for="results_cholesterol" class="block text-base font-medium text-gray-700 mb-1">Kolesterol
                                (mg/dL)</label>
                        ...
                        <!-- Eyes -->
                        <div class="w-full">
                            <label for="results_eyes" class="block text-base font-medium text-gray-700 mb-1">Pemeriksaan
                                Mata</label>
                        ...
                        <!-- Ears -->
                        <div class="w-full">
                            <label for="results_ears" class="block text-base font-medium text-gray-700 mb-1">Pemeriksaan
                                Telinga</label>
                        ...
                        <div class="w-full md:col-span-3">
                            <label for="results_note" class="block text-base font-medium text-gray-700 mb-1">Catatan Tambahan
                                / Diagnosis</label>
