...
<div class="w-full mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-3">
            <a href="{{ route('health-posts.edit', $healthPost) }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-200 text-gray-700 text-base font-medium rounded-lg hover:bg-gray-50 transition-colors shadow-sm">
            ...
            <div class="bg-white rounded-xl shadow-[0_0_20px_rgba(0,0,0,0.05)] border border-gray-100 overflow-hidden">
                ...
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Nama Posyandu</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Alamat</label>
                        <p class="text-gray-900 leading-relaxed">{{ $healthPost->address ?? '-' }}</p>
                    </div>
                     <div>
                        <label class="block text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Dibuat Pada</label>
                        <p class="text-gray-600 text-base">{{ $healthPost->created_at->format('d F Y, H:i') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="md:col-span-1 space-y-6">
             <div class="bg-gradient-to-br from-teal-500 to-teal-600 rounded-xl shadow-[0_5px_15px_rgba(20,184,166,0.3)] border border-transparent overflow-hidden text-white p-6 relative">
                 <div class="relative z-10">
                     ...
                     <p class="text-teal-100 text-base mb-4">Ringkasan data posyandu ini.</p>
                     
                     <div class="space-y-4">
                         <div class="bg-white/10 rounded-lg p-3 backdrop-blur-sm">
                             <div class="text-2xl font-bold">{{ $healthPost->staffs_count }}</div>
                             <div class="text-sm text-teal-100 uppercase">Total Kader</div>
                         </div>
                         <div class="bg-white/10 rounded-lg p-3 backdrop-blur-sm">
                             <div class="text-2xl font-bold">-</div>
                             <div class="text-sm text-teal-100 uppercase">Total Kegiatan</div>
                         </div>
                     </div>
                 </div>
