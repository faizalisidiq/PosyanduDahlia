    ...
    <div class="flex justify-end">
        <form action="{{ route('schedules.index') }}" method="GET" class="flex items-center gap-2">
            <input type="date" name="date" value="{{ $date }}" 
                class="block rounded-lg border-gray-200 bg-white text-gray-900 focus:border-teal-500 focus:ring-teal-500 shadow-sm sm:text-base p-2 transition-all">
            <button type="submit" class="px-4 py-2 bg-gray-800 text-white text-base font-medium rounded-lg hover:bg-gray-900 transition-colors">
                Filter
            </button>
        </form>
    </div>
                ...
                <tbody class="divide-y divide-gray-100 text-base">
                    @forelse($schedules as $schedule)
                        <tr class="hover:bg-gray-50/80 transition-colors">
                            ...
                            <td class="px-6 py-4 text-gray-600 font-mono text-sm whitespace-nowrap">{{ $schedule->child->mother->phone_number ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2.5 py-0.5 rounded-full text-sm font-medium bg-blue-50 text-blue-700 border border-blue-100">
                            ...
                            <td class="px-6 py-4 text-right whitespace-nowrap">
                                @if($schedule->child->mother->phone_number)
                                    ...
                                    @endphp
                                    <a href="{{ $waUrl }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm">
                                ...
                                @else
                                    <span class="text-sm text-gray-400">No WhatsApp tidak ada</span>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                <div class="flex flex-col items-center justify-center max-w-xs mx-auto">
                                    ...
                                    <h3 class="text-gray-900 font-medium text-base mb-1">Tidak ada jadwal pemeriksaan</h3>
                                    <p class="text-sm text-gray-500">Untuk tanggal {{ \Carbon\Carbon::parse($date)->format('d F Y') }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
